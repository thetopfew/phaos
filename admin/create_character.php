...
<?php
$result = mysqli_query($db, "SELECT * FROM phaos_characters WHERE username = '$username'");
...
if($create_char == "yes") {
	if($duplicate != "yes") {
		$races_lookup = mysqli_query($db, "SELECT * FROM phaos_races WHERE name='$race'");
		if ($row = mysqli_fetch_array($races_lookup)) {
			$race_name = $row["name"];
			$strength = $row["str"];
			$dexterity = $row["dex"];
			$wisdom = $row["wis"];
		...
		// skill adjustments:
		//echo "Class:".$class;
		$skillquery = mysqli_query($db, "SELECT * FROM phaos_classes WHERE name='$class'");
		//echo "query:".$skillquery;
		$skills=mysqli_fetch_array($skillquery);
		echo $skills;
      ...
      <td colspan="2"> 
		...
		if($duplicate == "yes") {
			...
			//DELETE CHARACETERS WITH BLANK NAMES
				...
				<select name="sex">
				<?php
				$sex = mysqli_query($db, "SELECT * FROM phaos_sex ORDER BY sex ASC");
				if ($row = mysqli_fetch_array($sex)) {
					do {
						$sex_name = $row["sex"];
						print ("<option value=\"$sex_name\">$sex_name</option>");
					} while ($row = mysqli_fetch_array($sex));
				}
				?>
				</select>
				</td>
				...
				<select name="race">
				<?php
				$races = mysqli_query($db, "SELECT * FROM phaos_races ORDER BY name ASC");
				if ($row = mysqli_fetch_array($races)) {
					do {
						$race_name = $row["name"];
				...
				<select name="class">
				<?php
				$classes = mysqli_query($db, "SELECT * FROM phaos_classes ORDER BY name ASC");
				if ($row = mysqli_fetch_array($classes)) {
					do {
						$class_name = $row["name"];
