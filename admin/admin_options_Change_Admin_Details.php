...
if(@$_REQUEST['changeme'])
{
    if($admin_pass === $admin_pass_2){
    	...
    	$result = mysqli_query($db, "UPDATE phaos_admin SET admin_user='$admin_user', admin_pass='$PHP_ADMIN_MD5PW' WHERE admin_user = '$PHP_ADMIN_USER'");
        if($result){
	...
	$self = mysqli_query($db, "SELECT * FROM phaos_admin WHERE id=1");
