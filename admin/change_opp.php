...
{
	mysqli_query($db, "UPDATE phaos_opponents SET name='$name', hit_points='$hit_points', race='$race', class='$class', min_damage='$min_damage', max_damage='$max_damage', AC='$AC', xp_given='$xp_given', gold_given='$gold_given', image_path='$image', location='$location' WHERE id='$id'");
...
{
	$self = mysqli_query($db, "SELECT * FROM phaos_opponents WHERE id=$opp");
	while ($row = mysqli_fetch_array($self)) {
		...
		$result = mysqli_query ("SELECT * FROM phaos_locations WHERE id = '$location_id'");
...
<input type="hidden" name="id" value="<?php echo $id; ?>">
  ...
  <table width="600" border="1" cellspacing="0" cellpadding="3">
    ...
    <tr> 
	  ...
	  <?php
	  $result = mysqli_query ("SELECT * FROM phaos_locations ORDER BY name ASC");
